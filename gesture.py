...
def get_action(coordinates, skip_factor=5, landmarks=None):
    ...
    for coordinate in coordinates:
        ...
        for i in range(len(coordinate)):
            area += (coordinate[j][0]+coordinate[i][0]) * \
    ...
    normalized_landmarks = (landmarks - np.array([landmark_mean_x, landmark_mean_y]))/np.max(
        np.abs([landmarks[:, 0], landmarks[:, 1]]), axis=1)
    ...
    if mean_angle > 0.1:
        bpy.ops.transform.rotate(
            value=mean_angle, orient_axis=axis_s[axis], orient_type='GLOBAL')
        bpy.ops.wm.redraw_timer(type="DRAW_WIN_SWAP", iterations=1)
    elif mean_angle < -0.1:
        bpy.ops.transform.rotate(
            value=mean_angle, orient_axis=axis_s[axis], orient_type='GLOBAL')
...
class ButtonOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for i in range(1000):
            ...
            landmark_list = detector.get_position(
                img, draw=False, mark_tracker=8)
            if len(landmark_list) != 0:
                ...
                normalized_finger_tips = (finger_tips - np.array([x_mean, y_mean]))/np.max(
                    np.abs([finger_tips[:, 0], finger_tips[:, 1]]), axis=1)
