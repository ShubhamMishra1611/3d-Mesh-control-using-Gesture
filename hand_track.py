...
class TrackHand:
    def __init__(self, mode=False, max_hands=2, model_complexity=0, detection_confidence=0.5, tracking_confidence=0.5) -> None:
        ...
        self.hands = self.mpHand.Hands(
            self.mode, self.max_hands, self.model_complexity, self.detection_confidence, self.tracking_confidence)
            ...
            for hand in self.results.multi_hand_landmarks:
                if draw_markers:
                    self.mpDraw.draw_landmarks(
                        img, hand, self.mpHand.HAND_CONNECTIONS)
        return img

    def get_position(self, img, hand_number=0, draw=True, mark_tracker=8):
        if mark_tracker not in list(range(0, 21)):
            mark_tracker = 8  # if wrong value then track index finger
        land_marks = []
        if self.results.multi_hand_landmarks:
